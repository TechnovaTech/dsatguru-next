...
import DefaultTestimonailImg from "../../assets/hero-3.png";
...
const EnrollmentPage = () => {
  ...
  useEffect(() => {
    if (courses.length > 0) {
      ...
      console.log('Banner image URL:', foundCourse?.bannerImageUrl);
      setCourse(foundCourse);
    }
  }, [courseId, courses]);

  useEffect(() => {
    // Force refresh course data when component mounts
    refreshCourses();
  }, [courseId]);
  ...
  const handleEnroll = async (scheduleId = null) => {
    if (!localStorage.getItem("authToken")) {
      navigate("/login", { state: { returnTo: `/enrollment/${courseId}` } });
      return;
    }

    try {
      setEnrollLoading(true);
      // If already enrolled, navigate directly to course
      try {
        const { checkEnrollment } = await import('../../services/api/courses');
        const already = await checkEnrollment(courseId);
        if (already) {
      ...
      const response = await createCheckoutSession(courseId, scheduleId, '/dashboard/courses');
  ...
  const handleScheduleSelection = () => {
    if (!isAuthenticated()) {
      ...
      navigate("/login", { state: { courseId, courseSlug } });
      return;
    }
    ...
    if (course.courseSchedules?.length === 1) {
      ...
      // If no schedules, just enroll without a schedule ID
      handleEnroll(null);
    }
  };
    ...
    <section className="w-full bg-gradient-to-b from-blue-50 to-white">
      ...
      <div className="relative w-full min-h-[70vh] overflow-hidden bg-gray-900">
        <div className="absolute inset-0 flex md:justify-end justify-center">
          <img
            src={course.bannerImageUrl || course.bannerUrl || Hero3Img}
            alt="Background"
            className="w-full md:w-1/2 h-full object-cover opacity-30 object-center"
            onLoad={() => console.log('Banner image loaded:', course.bannerImageUrl)}
            onError={() => console.log('Banner image failed to load:', course.bannerImageUrl)}
          />
          ...
          <div className="bg-white rounded-3xl shadow-2xl overflow-hidden md:col-span-2 flex flex-col">
            ...
            >
              ...
              <div className="w-full h-64 overflow-hidden">
                <img
                  src={course.bannerImageUrl || course.bannerUrl || DefaultTestimonailImg}
