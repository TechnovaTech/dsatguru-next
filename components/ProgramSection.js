...
const ProgramCard = ({ data, index, headerHeight, isEnrolled }) => {
  ...
  const discountPercentage = useMemo(() => {
    if (data.price && data.discountedPrice) {
      const discount = ((Number(data.price) - Number(data.discountedPrice)) / Number(data.price)) * 100
      ...
      >
        {courses.map((course, i) => (
          <ProgramCard
            ...
            isEnrolled={enrolledIds.includes(course._id) || enrolledIds.includes(course.id)}
          />
        ))}
      </motion.div>
