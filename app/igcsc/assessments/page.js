...
const TYPES = [
  { key: 'APT', name: 'Aptitude Test (APT)', icon: FiZap, tone: 'from-indigo-500 to-blue-600',
    desc: 'Timed reasoning & aptitude assessment to benchmark a student’s baseline ability.' },
  { key: 'SMS', name: 'Subject Mastery Set (SMS)', icon: FiLayers, tone: 'from-violet-500 to-purple-600',
    desc: 'Topic-wise mastery checks that track depth of understanding per IGCSE subject.' },
  { key: 'CSQ', name: 'Concept & Skill Quiz (CSQ)', icon: FiEdit3, tone: 'from-emerald-500 to-teal-600',
    desc: 'Short, focused quizzes for quick concept reinforcement and skill drills.' },
]
...
export default function AssessmentsPage() {
  ...
  useEffect(() => {
    (async () => { setTests(await apiGetSafe('/api/igcsc/tests', [])); setLoading(false) })()
  }, [])

  if (loading) return <Loading label="Loading assessments…" />
  const countFor = (key) => tests.filter((t) => t.testType === key).length

  return (
    <div>
      <PageHeader title="APT · SMS · CSQ Assessments" subtitle="Structured IGCSE assessment formats built on the IGCSC test engine." />
        ...
        {TYPES.map((type) => (
          <div key={type.key} className="flex flex-col rounded-2xl border border-slate-200 bg-white p-6 shadow-sm transition-shadow hover:shadow-md">
            <div className="flex items-center justify-between">
              ...
              <Badge tone="indigo">{countFor(type.key)} live</Badge>
            </div>
            ...
            <Link href="/igcsc/tests" className="mt-4 flex items-center justify-center gap-1.5 rounded-lg bg-slate-900 px-4 py-2 text-sm font-semibold text-white transition-colors hover:bg-slate-800">Manage {type.key} <FiArrowRight size={14} /></Link>
          </div>
        ))}
      </div>
      <Card className="mt-6" title="How these formats work">
        <div className="grid grid-cols-1 gap-4 p-5 text-sm text-slate-600 sm:grid-cols-3">
          ...
          <p><span className="font-semibold text-slate-800">SMS</span> runs per subject/topic — confirm IGCSE mastery before moving on.</p>
