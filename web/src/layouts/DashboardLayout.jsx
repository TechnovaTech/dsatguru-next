...
export default function DashboardLayout({ children }) {
  ...
  return (
    <div className="flex min-h-screen bg-gray-50 font-[Poppins] overflow-hidden">
      {isOpen && (
        <div className="fixed inset-0 z-40 bg-black/50 md:hidden" onClick={() => setIsOpen(false)}></div>
      )}
      <aside className={`h-screen fixed z-50 md:static top-0 right-0 bg-white text-gray-800 w-64 p-6 space-y-6 flex flex-col justify-between transform transition-transform duration-300 ease-in-out shadow-lg ${isOpen ? "translate-x-0" : "translate-x-full md:translate-x-0"}`}>
        <div>
          <div className="flex items-center justify-between gap-3 mb-8">
            <Link to="/" className="text-2xl font-bold text-blue-700 tracking-wide">DSATGURU</Link>
          ...
          <nav className="space-y-3">
            {navItems.map((item) => (
              <Link to={item.path} key={item.name} onClick={() => setIsOpen(false)} className={`flex items-center gap-3 px-4 py-2 rounded-md text-sm font-medium transition-all duration-200 hover:bg-blue-100 ${location.pathname === item.path ? "bg-blue-100" : ""}`}>
                {item.icon} {item.name}
              </Link>
            ))}
          </nav>
        ...
        <div className="mt-6 pt-6 border-t border-gray-200">
          <div className="flex items-center gap-3 mb-3">
            <div className="w-10 h-10 bg-blue-600 text-white rounded-full flex items-center justify-center text-sm font-semibold">{initials}</div>
            <div>
