...
// Within a domain the SKILL order matters too, and it is not arbitrary: a real R&W
// module opens with the short fill-in-the-blank vocabulary items and only then moves
// to the passage-based ones. Sorting by domain alone let a paragraph question open
// the module, which is what students noticed.
//
// Derived from both pattern workbooks — mean position of each skill inside its
// module across all 28 module-sheets (scratchpad/vrf/skill-order.py):
//   Words in Context 0.108 (n=65)   Text Structure and Purpose 0.245 (n=42)
//   Central Ideas 0.333 (n=5)       Command of Evidence Quant 0.439 (n=13)
//   Inferences 0.442 (n=75)         Command of Evidence Textual 0.454 (n=8)
//   Form, Structure and Sense 0.665 (n=63)   Boundaries 0.667 (n=8)
//   Transitions 0.815 (n=39)        Rhetorical Synthesis 0.943 (n=57)
// Where the sample is thin or two skills are statistically tied, the College Board
// listing order breaks the tie.
export const RW_SKILL_ORDER = [
  'Words in Context',                    // short blanks open the module
  ...
  'Transitions',
  'Rhetorical Synthesis',               // notes-based synthesis closes it
]
...
export function composeDsatModule(pool, section, tier, usedIds, opts = {}) {
  ...
  // Ordering: R&W follows the real exam's domain order AND, inside each domain, its
  // skill order — the short fill-in-the-blank vocabulary items lead, the passage-based
  // ones follow. Math ascends by difficulty.
  if (!isMath) {
    const dRank = d => { const i = RW_DOMAIN_ORDER.indexOf(d); return i === -1 ? RW_DOMAIN_ORDER.length : i }
    const sRank = q => { const i = RW_SKILL_ORDER.indexOf(skillOf(q)); return i === -1 ? RW_SKILL_ORDER.length : i }
    picked.sort((a, b) => (dRank(a.domain) - dRank(b.domain)) || (sRank(a.q) - sRank(b.q)))
  } else {
