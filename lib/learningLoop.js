...
// user+question+session (skips rows that already exist). Fully wrapped in try/catch:
...
export async function syncWrongAnswers(userId, session) {
  try {
    ...
    // Idempotency: skip any (user, question, session) that already has an ErrorLog row.
