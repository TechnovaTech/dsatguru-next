// Shared SAT performance analysis: turns a flat list of answered questions into
// the Subject -> Content Domain -> Skill breakdown used by every results and
// analysis screen, so all of them agree on the numbers and the wording.
//
// The taxonomy itself lives in lib/dsatDomains.js (single source of truth for the
// canonical domain and skill names stored on each Question). This module adds the
// College Board *blueprint* metadata that a score report shows next to it:
// the operational weight and question count each content domain carries on a real
// Digital SAT — from the "Digital SAT Suite of Assessments" content tables
// (R&W Table 2, Math Table 3).

import { RW_SKILLS, MATH_SKILLS, DOMAIN_SKILLS, SKILL_TO_DOMAIN } from './dsatDomains'

export const SUBJECT_RW = 'Reading and Writing'
export const SUBJECT_MATH = 'Math'

// Per-domain operational weight + question count on a full-length test.
export const DOMAIN_BLUEPRINT = {
  // Reading and Writing — 54 operational questions
  'Craft and Structure': { weight: '~28%', questions: '13–15', order: 1, subject: SUBJECT_RW },
  'Information and Ideas': { weight: '~26%', questions: '12–14', order: 2, subject: SUBJECT_RW },
  'Standard English Conventions': { weight: '~26%', questions: '11–15', order: 3, subject: SUBJECT_RW },
  'Expression of Ideas': { weight: '~20%', questions: '8–12', order: 4, subject: SUBJECT_RW },
  // Math — 44 operational questions
  Algebra: { weight: '~35%', questions: '13–15', order: 1, subject: SUBJECT_MATH },
  'Advanced Math': { weight: '~35%', questions: '13–15', order: 2, subject: SUBJECT_MATH },
  'Problem-Solving and Data Analysis': { weight: '~15%', questions: '5–7', order: 3, subject: SUBJECT_MATH },
  'Geometry and Trigonometry': { weight: '~15%', questions: '5–7', order: 4, subject: SUBJECT_MATH },
}

// Short labels for tight spaces (radar axes, chips).
export const DOMAIN_SHORT = {
  'Craft and Structure': 'Craft & Structure',
  'Information and Ideas': 'Information & Ideas',
  'Standard English Conventions': 'Standard English',
  'Expression of Ideas': 'Expression of Ideas',
  Algebra: 'Algebra',
  'Advanced Math': 'Advanced Math',
  'Problem-Solving and Data Analysis': 'Problem-Solving & Data',
  'Geometry and Trigonometry': 'Geometry & Trig',
}

// Performance bands — the legend shown under every breakdown.
export const BANDS = [
  { key: 'strong', label: 'Strong', hint: '70% and above', min: 70, dot: '#16a34a', bar: '#16a34a', text: 'text-emerald-700', bg: 'bg-emerald-50', ring: 'border-emerald-200' },
  { key: 'developing', label: 'Developing', hint: '40%–69%', min: 40, dot: '#eab308', bar: '#eab308', text: 'text-amber-700', bg: 'bg-amber-50', ring: 'border-amber-200' },
  { key: 'needs', label: 'Needs Improvement', hint: 'Below 40%', min: 0, dot: '#dc2626', bar: '#dc2626', text: 'text-rose-700', bg: 'bg-rose-50', ring: 'border-rose-200' },
]

// The band a percentage falls in. `null` (no questions seen) has no band.
export function bandFor(pct) {
  if (pct == null || Number.isNaN(pct)) return null
  for (const b of BANDS) if (pct >= b.min) return b
  return BANDS[BANDS.length - 1]
}

const isMathSubject = (s) => String(s || '').toLowerCase().includes('math')

// The four states a question can end a test in. Kept separate from `correct`
// because an omitted question is not the same as a wrong one when you are
// diagnosing a student, even though both score zero.
export const STATUS = { CORRECT: 'correct', INCORRECT: 'incorrect', OMITTED: 'omitted', UNSEEN: 'unseen' }

const normDifficulty = (d) => {
  const s = String(d || '').trim().toLowerCase()
  return s === 'easy' ? 'Easy' : s === 'hard' ? 'Hard' : s === 'medium' ? 'Medium' : ''
}

// Normalise one question into { subject, domain, skill, correct, status, seconds, difficulty }.
// Accepts the shapes the various result APIs already produce.
export function normalizeRow(row) {
  if (!row) return null
  const skill = String(row.skill || '').trim()
  const storedDomain = String(row.domain || '').trim()
  // The skill is authoritative: it always determines its domain.
  const domain = (skill && SKILL_TO_DOMAIN[skill]) || (DOMAIN_SKILLS[storedDomain] ? storedDomain : '')
  if (!domain) return null
  const subject = DOMAIN_BLUEPRINT[domain]?.subject
    || (isMathSubject(row.subject) ? SUBJECT_MATH : SUBJECT_RW)
  const correct = row.correct != null ? !!row.correct
    : row.isCorrect != null ? !!row.isCorrect
    : false

  // An explicit `answered` beats inference; otherwise read the answer field the
  // result views carry. When a caller supplies NO answer information at all the row
  // came straight from a response list, so it was answered — defaulting to "omitted"
  // there would relabel every wrong answer as a skip.
  const answerVal = row.userAnswer !== undefined ? row.userAnswer : row.selectedAnswer
  const answered = row.answered != null
    ? !!row.answered
    : answerVal === undefined
      ? true
      : (answerVal != null && String(answerVal).trim() !== '')
  // `wasVisited` is only set by the result view; elsewhere every row IS a response.
  const visited = row.wasVisited != null ? !!row.wasVisited : true

  let status = STATUS.UNSEEN
  if (correct) status = STATUS.CORRECT
  else if (answered) status = STATUS.INCORRECT
  else if (visited) status = STATUS.OMITTED

  const seconds = Math.max(0, Number(row.timeSpent) || 0)
  return {
    subject,
    domain,
    skill: skill && SKILL_TO_DOMAIN[skill] === domain ? skill : '',
    correct,
    status,
    answered,
    seconds,
    difficulty: normDifficulty(row.difficulty),
  }
}

const pctOf = (c, t) => (t > 0 ? Math.round((c / t) * 100) : null)

// Shared per-bucket rollup: accuracy plus the diagnostic counters a score report
// shows next to it. Accuracy is over questions the student actually SAW, so an
// adaptive test that never served an item cannot drag a skill down.
function rollup(rows) {
  const seen = rows.filter((r) => r.status !== STATUS.UNSEEN)
  const correct = seen.filter((r) => r.correct).length
  const incorrect = seen.filter((r) => r.status === STATUS.INCORRECT).length
  const omitted = seen.filter((r) => r.status === STATUS.OMITTED).length
  const unseen = rows.length - seen.length
  const timed = seen.filter((r) => r.seconds > 0)
  const totalSeconds = timed.reduce((a, r) => a + r.seconds, 0)
  return {
    correct,
    incorrect,
    omitted,
    unseen,
    total: seen.length,
    pct: pctOf(correct, seen.length),
    // Average seconds per question, over the questions that actually carry a time.
    avgSeconds: timed.length ? Math.round(totalSeconds / timed.length) : null,
    totalSeconds,
  }
}

// Build the full breakdown.
//   rows: [{ subject, domain, skill, isCorrect|correct }]
//   opts.includeEmpty: also list skills the student saw no questions from
// Returns { subjects: [...], overall, hasData }
export function buildSatBreakdown(rows, opts = {}) {
  const includeEmpty = !!opts.includeEmpty
  const supplied = (rows || []).filter(Boolean).length
  const clean = (rows || []).map(normalizeRow).filter(Boolean)
  // Rows we could not place in the taxonomy at all. Reported so a caller can say so
  // rather than silently showing a total that disagrees with the page around it.
  const unclassified = supplied - clean.length

  const subjects = []
  for (const [subjectName, skillMap] of [[SUBJECT_RW, RW_SKILLS], [SUBJECT_MATH, MATH_SKILLS]]) {
    const subjectRows = clean.filter((r) => r.subject === subjectName)
    const domains = []
    for (const domainName of Object.keys(skillMap)) {
      const domainRows = subjectRows.filter((r) => r.domain === domainName)
      const skills = []
      for (const skillName of skillMap[domainName]) {
        const sr = domainRows.filter((r) => r.skill === skillName)
        if (!sr.length && !includeEmpty) continue
        const st = rollup(sr)
        skills.push({ skill: skillName, ...st, band: bandFor(st.pct) })
      }
      // Questions that resolved to this domain but carry no canonical skill still
      // count toward the domain, so they get their own row — otherwise the skill
      // rows would not add up to the domain total sitting above them.
      const unskilledRows = domainRows.filter((r) => !r.skill)
      const unskilled = unskilledRows.length
      if (unskilled > 0) {
        const ust = rollup(unskilledRows)
        skills.push({ skill: null, isOther: true, ...ust, band: bandFor(ust.pct) })
      }
      const dst = rollup(domainRows)
      if (!domainRows.length && !includeEmpty) continue
      domains.push({
        domain: domainName,
        short: DOMAIN_SHORT[domainName] || domainName,
        ...DOMAIN_BLUEPRINT[domainName],
        skills,
        unskilled,
        ...dst,
        band: bandFor(dst.pct),
      })
    }
    const sst = rollup(subjectRows)
    subjects.push({
      subject: subjectName,
      isMath: subjectName === SUBJECT_MATH,
      domains: domains.sort((a, b) => (a.order || 9) - (b.order || 9)),
      ...sst,
      band: bandFor(sst.pct),
    })
  }

  const ost = rollup(clean)
  return {
    subjects: subjects.filter((s) => s.total > 0 || includeEmpty),
    overall: ost,
    // Correct / incorrect / omitted split by question difficulty, for the
    // "Difficulty Analysis" chart.
    difficulty: ['Easy', 'Medium', 'Hard'].map((d) => {
      const rows2 = clean.filter((r) => r.difficulty === d)
      const st = rollup(rows2)
      return { difficulty: d, ...st, missed: st.omitted + st.unseen }
    }).filter((d) => d.total > 0 || d.missed > 0 || includeEmpty),
    unclassified,
    // True only when the student actually SAW something. A set of rows that are all
    // "never served" has nothing to report, and rendering it would print a bare "%".
    hasData: ost.total > 0,
  }
}

// The student's strongest / weakest areas, for the summary callouts.
//   minQuestions guards against a 1-question "100%" looking like a strength.
export function highlights(breakdown, opts = {}) {
  const minQuestions = opts.minQuestions ?? 2
  const all = []
  for (const s of breakdown?.subjects || []) {
    for (const d of s.domains) {
      for (const sk of d.skills) {
        if (sk.total >= minQuestions && sk.pct != null) all.push({ ...sk, domain: d.domain, subject: s.subject })
      }
    }
  }
  const byPct = [...all].sort((a, b) => a.pct - b.pct || b.total - a.total)
  return {
    weakest: byPct.filter((x) => x.pct < 70).slice(0, 4),
    strongest: [...byPct].reverse().filter((x) => x.pct >= 70).slice(0, 3),
    strongestDomain: (breakdown?.subjects || [])
      .flatMap((s) => s.domains)
      .filter((d) => d.total >= minQuestions && d.pct != null)
      .sort((a, b) => b.pct - a.pct)[0] || null,
  }
}

// One-paragraph takeaway, phrased for a student. Deterministic — no model needed.
export function takeaway(breakdown) {
  if (!breakdown?.hasData) return ''
  const { weakest, strongest } = highlights(breakdown)
  const list = (arr) => arr.map((x) => x.skill).filter(Boolean).join(', ')
  if (!weakest.length) {
    return strongest.length
      ? `Strong work across the board — you are at 70% or better in every skill with enough questions to judge, including ${list(strongest)}. Keep the volume up and push into harder items.`
      : 'Keep practising to build up enough attempts per skill for a reliable read on your strengths.'
  }
  const strongPart = strongest.length ? `Keep building on ${list(strongest)}. ` : ''
  return `${strongPart}Focus next on ${list(weakest)} — these are the skills costing you the most points right now.`
}

// Every content domain the student saw, flattened — the row set behind the
// "Unit Level Analysis" cards and the accuracy / speed charts.
export function unitRows(breakdown) {
  const out = []
  for (const s of breakdown?.subjects || []) {
    for (const d of s.domains) {
      if (!d.total) continue
      out.push({
        unit: d.domain,
        short: d.short,
        subject: s.subject,
        isMath: s.isMath,
        accuracy: d.pct,
        avgSeconds: d.avgSeconds,
        correct: d.correct,
        incorrect: d.incorrect,
        omitted: d.omitted,
        unseen: d.unseen,
        missed: d.omitted + d.unseen,
        total: d.total,
        band: d.band,
      })
    }
  }
  return out
}

// Points for the proficiency bubble chart: accuracy on x, average latency on y,
// bubble sized by how many questions back the point up.
export function proficiencyPoints(breakdown) {
  return unitRows(breakdown)
    .filter((u) => u.accuracy != null && u.avgSeconds != null)
    .map((u) => ({ x: u.accuracy, y: u.avgSeconds, z: u.total, unit: u.unit, short: u.short, isMath: u.isMath }))
}

// Merge this student's per-unit numbers with the cohort's best and average, for the
// comparison charts. `cohort` is { unit: { bestAccuracy, avgAccuracy, bestSeconds, avgSeconds } }.
export function comparisonRows(breakdown, cohort) {
  if (!cohort) return []
  return unitRows(breakdown).map((u) => {
    const c = cohort[u.unit] || {}
    return {
      unit: u.unit,
      short: u.short,
      you: u.accuracy,
      best: c.bestAccuracy ?? null,
      average: c.avgAccuracy ?? null,
      youSeconds: u.avgSeconds,
      bestSeconds: c.bestSeconds ?? null,
      averageSeconds: c.avgSeconds ?? null,
    }
  })
}

// Correct / incorrect / missed counts as PERCENTAGES of the bucket, for the stacked
// "Performance Distribution" bar. Percentages so bars of different lengths align.
export function distribution(bucket) {
  const total = (bucket?.correct || 0) + (bucket?.incorrect || 0) + (bucket?.omitted || 0) + (bucket?.unseen || 0)
  if (!total) return { correct: 0, incorrect: 0, missed: 0, total: 0 }
  const missed = (bucket.omitted || 0) + (bucket.unseen || 0)
  return {
    correct: ((bucket.correct || 0) / total) * 100,
    incorrect: ((bucket.incorrect || 0) / total) * 100,
    missed: (missed / total) * 100,
    total,
  }
}

// How many Easy / Medium / Hard questions a bucket held — the little three-bar
// difficulty sparkline next to each topic row.
export function difficultySpark(rows) {
  const out = { Easy: 0, Medium: 0, Hard: 0 }
  for (const r of rows || []) if (out[r.difficulty] != null) out[r.difficulty]++
  const max = Math.max(1, out.Easy, out.Medium, out.Hard)
  return { counts: out, max }
}

// Runs of consecutive correct / incorrect answers across the test, in order.
// Omitted and never-served questions break a run without belonging to either side.
export function streaks(rows) {
  const seq = (rows || []).map(normalizeRow).filter(Boolean).filter((r) => r.status !== STATUS.UNSEEN)
  const runs = []
  let cur = null
  seq.forEach((r, i) => {
    const kind = r.status === STATUS.CORRECT ? 'correct' : r.status === STATUS.INCORRECT ? 'incorrect' : null
    if (kind && cur && cur.kind === kind) { cur.length++; cur.end = i + 1; return }
    if (cur) runs.push(cur)
    cur = kind ? { kind, length: 1, start: i + 1, end: i + 1 } : null
  })
  if (cur) runs.push(cur)
  const best = (kind) => runs.filter((r) => r.kind === kind).reduce((m, r) => Math.max(m, r.length), 0)
  return { runs, longestCorrect: best('correct'), longestIncorrect: best('incorrect'), answered: seq.length }
}

// Time spent per question in test order, with the running total — the "Pace" view.
export function paceData(rows) {
  const seq = (rows || []).map(normalizeRow).filter(Boolean)
  let running = 0
  return seq.map((r, i) => {
    running += r.seconds
    return {
      number: i + 1,
      seconds: r.seconds,
      cumulativeMinutes: Math.round((running / 60) * 10) / 10,
      status: r.status,
      domain: r.domain,
      skill: r.skill,
    }
  })
}

// Per-question time against the cohort's average for that question — the
// "Exceed Time" view. `cohortAvgById` is { questionId: avgSeconds }.
export function exceedTimeData(rows, cohortAvgById) {
  const out = []
  ;(rows || []).forEach((raw, i) => {
    const r = normalizeRow(raw)
    if (!r || r.seconds <= 0) return
    const id = String(raw._id || raw.questionId || '')
    const avg = cohortAvgById ? Number(cohortAvgById[id]) : null
    out.push({
      number: i + 1,
      seconds: r.seconds,
      average: Number.isFinite(avg) && avg > 0 ? Math.round(avg) : null,
      exceeded: Number.isFinite(avg) && avg > 0 ? r.seconds > avg : false,
      status: r.status,
      domain: r.domain,
      skill: r.skill,
    })
  })
  return out
}

// mm:ss for a duration in seconds.
export function formatDuration(totalSeconds) {
  const s = Math.max(0, Math.round(Number(totalSeconds) || 0))
  const h = Math.floor(s / 3600)
  const m = Math.floor((s % 3600) / 60)
  const sec = s % 60
  if (h > 0) return `${h} hr ${m} min ${sec} sec`
  if (m > 0) return `${m} min ${sec} sec`
  return `${sec} sec`
}

// Data for the by-domain radar chart (recharts RadarChart).
export function radarData(breakdown) {
  const out = []
  for (const s of breakdown?.subjects || []) {
    for (const d of s.domains) {
      if (d.pct == null) continue
      out.push({ domain: d.short, full: d.domain, subject: s.subject, score: d.pct, total: d.total })
    }
  }
  return out
}

// A rough SAT-style section score (200–800) from raw accuracy, used ONLY when the
// caller has no real scaled score to show. Clearly an estimate, never a claim of
// an official score.
export function estimateSectionScore(pct) {
  if (pct == null) return null
  return Math.round((200 + (pct / 100) * 600) / 10) * 10
}
